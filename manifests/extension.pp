...
define vscode::extension (
    ...
    if $ensure != 'absent' {
      exec { "install-vscode-extension-${extension_name}":
        ...
        unless    => "cmd.exe /c \"code.cmd --list-extensions --show-versions | findstr.exe /i ${extension_name}\"",
      ...
      exec { "uninstall-vscode-extension-${extension_name}":
        ...
        onlyif    => "cmd.exe /c \"code.cmd --list-extensions --show-versions | findstr.exe /i ${extension_name}\"",
        path      => 'C:/Windows/System32;C:/Program Files (x86)/Microsoft VS Code/bin',
        logoutput => true,
      }
  }
